...
def create_edge_attr(df):
    ...
    for i in range(nrow):
        for j in range(ncol):
            if df.iloc[i, j]!= ' ' and df.iloc[i, j] != 'NA':
                edge_attr.append([float(df.iloc[i, j])])
            else: edge_attr.append(-1)
    edge_attr = edge_attr + edge_attr
    return edge_attr


def get_data(df_X, df_y, node_mode, train_edge_prob, split_sample_ratio, split_by, train_y_prob, seed=0,
             normalize=True):
    if len(df_y.shape) == 1:
        df_y = df_y.to_numpy()
    ...
    for i in range(nrow):
        for j in range(ncol):
            if df_X.iloc[i, j] != '' and df_X.iloc[i,j] != 'NA':
...
#    test_y_mask = ~train_y_mask
    ...
    for i in range(y.shape[0]):
        if df_y.iloc[i] != '' and df_y.iloc[i] != 'NA':
            train_edge_mask.append(True)
        else:
            train_edge_mask.append(False)
    test_y_mask = ~train_y_mask
...
def load_data(args):
    ...
    if not hasattr(args, 'split_sample'):
        args.split_sample = 0
    data = get_data(df_X, df_y, args.node_mode, args.train_edge, args.split_sample, args.split_by, args.train_y,
                    args.seed)
    return data
