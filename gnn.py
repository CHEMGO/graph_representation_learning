...
def train_gnn_mdi(data, args, log_path, device=torch.device('cpu')):
    ...
    # 将结果写入csv文件里
    pd_data = pd.DataFrame(pred_origin, columns=["gender","age","bmi","bloodGlucose","proinsulin","Cp120","diabetesPredigreeFunction","trainOutcome","dataSource","outcome"])
    ...
    pd_data_origin = pd.DataFrame(df,columns=["gender", "age", "bmi", "bloodGlucose", "proinsulin", "Cp120", "diabetesPredigreeFun","trainOutcome","dataSource","outcome"])
    ...
    pd_data_origin.to_csv("pd_data_origin_y_421.csv",index = False, float_format = '%.04f')