...
def train_gnn_mdi(data, args, log_path, device=torch.device('cpu')):
    ...
    for epoch in range(args.epochs):
        ...
        # print('length is: ')
        # print(len(pred))
        train = np.array(train_labels).reshape(-1,1)
        test = np.array(test_labels).reshape(-1,1)
        min_max_scaler = data.min_max_scaler
        origin_labels = np.append(train,test)
        # origin_labels = origin_labels.reshape(-1,1)
        origin_labels = origin_labels.reshape(-1, 8)
        train_origin = min_max_scaler.inverse_transform(origin_labels)
        print('origin train is :')
        print(train_origin)
